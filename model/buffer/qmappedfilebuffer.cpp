...
bool QMappedFileBuffer::read(QIODevice* iodevice)
{
    ...
    if(!m_device || !QDeviceBuffer::read(iodevice)) return false;
...
{
    if(iodevice == m_device) this->remap();
    else iodevice->write(reinterpret_cast<const char*>(m_mappeddata), m_device->size());
}
...
void QMappedFileBuffer::remap()
{
    ...
    if(m_mappeddata) f->unmap(m_mappeddata);
